...
baseLocal="/inside/depot/tcgafiles/ftp_auth/distro_ftpusers/anonymous/tumor/"

python tcgaManifest.py $baseURL open tcga.public.manifest 2>>web_error
rm -rf /inside/depot/tcgafiles/ftp_auth/distro_ftpusers/anonymous/tumor/*/bcr/biotab/clin/
python checkManifest.py tcga.public.manifest > tcga.public.status
grep missing_dir tcga.public.status | awk '{print $2}' | xargs -i mkdir -p /inside/depot/{}
grep missing_file tcga.public.status | awk '{print $2}' | xargs -P 4 -i curl https://tcga-data.nci.nih.gov/{} -o /inside/depot/{}  
